# Testing Notes

## Lab validation completed

- Connected Microsoft Graph successfully from PowerShell and Cursor terminal
- Retrieved users from the lab tenant
- Retrieved sign-in logs from the lab tenant
- Retrieved groups from the lab tenant
- Retrieved disabled users from the lab tenant
- Retrieved licensed users from the lab tenant
- Executed authentication method summary module
- Retrieved directory role summary
- Retrieved directory role members
- Generated executive summary output
- Exported `UserInventory.csv` successfully
- Exported `SignInSummary.csv` successfully
- Exported `GroupInventory.csv` successfully
- Exported `DisabledUserInventory.csv` successfully
- Exported `LicensedUserInventory.csv` successfully
- Exported `AuthenticationMethodSummary.csv` successfully
- Exported `DirectoryRoleSummary.csv` successfully
- Exported `DirectoryRoleMembers.csv` successfully
- Exported `ExecutiveSummary.md` successfully
- Verified disabled account sign-in failure appears in sign-in logs
- Verified settings-based execution through `config/settings.json`
- Verified Global Administrator role appears in directory role reporting
## Signing validation completed

- Verified PowerShell Authenticode signature on `src/main.ps1`
- Verified PowerShell Authenticode signatures on all `src/modules/*.psm1` files
- Confirmed signed scripts execute under `AllSigned` execution policy in the test session
- Confirmed sensitive certificate files such as `.pfx` and `.p12` are excluded from Git tracking
- Confirmed GitHub verified signed commit workflow is active
- Confirmed release/tag workflow has been updated for signed release publishing